defmodule Prikke.AccountsTest do
  ...
  describe "api_keys" do
    ...
    test "verify_api_key/1 validates key and returns organization and key name", %{user: user, org: org} do
      ...
      assert {:ok, verified_org, key_name} = Accounts.verify_api_key(full_key)
  ...
  describe "organization invites - member limits" do
    test "free tier enforces 2 member limit" do
      ...
      assert {:ok, _invite, _token} =
               Accounts.create_organization_invite(org, owner, %{
      ...
      assert "You've reached the maximum number of team members" <> _ =
               hd(errors_on(changeset).base)
    end
