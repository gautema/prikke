defmodule Prikke.TasksTest do
  ...
  describe "create_task_with_execution/4" do
    test "creates both task and execution in one call" do
      org = organization_fixture()
      scheduled_for = DateTime.utc_now() |> DateTime.add(60, :second) |> DateTime.truncate(:second)
      ...
      assert {:ok, task, execution} = Tasks.create_task_with_execution(org, attrs, scheduled_for)

      assert task.name == "CTE Task"
      assert task.url == "https://example.com/webhook"
      assert task.method == "POST"
      assert task.schedule_type == "once"
      assert task.organization_id == org.id
    ...
    test "sets callback_url on execution when provided" do
      org = organization_fixture()
      scheduled_for = DateTime.utc_now() |> DateTime.add(60, :second) |> DateTime.truncate(:second)
      ...
      assert {:ok, _task, execution} =
               Tasks.create_task_with_execution(org, attrs, scheduled_for,
                 callback_url: "https://example.com/callback"
               )
    ...
    test "sets queue on both task and execution" do
      org = organization_fixture()
      scheduled_for = DateTime.utc_now() |> DateTime.add(60, :second) |> DateTime.truncate(:second)
      ...
      assert task.queue == "payments"
      assert execution.queue == "payments"
    end
    ...
    test "returns error changeset for missing URL" do
      org = organization_fixture()
      scheduled_for = DateTime.utc_now() |> DateTime.add(60, :second) |> DateTime.truncate(:second)
    ...
    test "task and execution are queryable after creation" do
      org = organization_fixture()
      scheduled_for = DateTime.utc_now() |> DateTime.add(60, :second) |> DateTime.truncate(:second)
      ...
      {:ok, task, execution} = Tasks.create_task_with_execution(org, attrs, scheduled_for)

      # Task is in the DB
      db_task = Prikke.Repo.get(Task, task.id)
      assert db_task != nil
      assert db_task.name == "Queryable CTE"

      # Execution is in the DB
      db_exec = Prikke.Executions.get_execution(execution.id)
      assert db_exec != nil
      assert db_exec.task_id == task.id
      assert db_exec.status == "pending"
    end
    ...
    test "uses default values for optional fields" do
      ...
      assert {:ok, task, _execution} = Tasks.create_task_with_execution(org, attrs, scheduled_for)

      # method defaults to schema default "GET" when not explicitly set
      assert task.method == "GET"
      assert task.retry_attempts == 5
      assert task.timeout_ms == 30_000
      assert task.headers == %{}
    end
  end
