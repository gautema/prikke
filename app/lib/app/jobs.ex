defmodule Prikke.Jobs do
    ...
    case String.split(cron, " ") do
      [minute, hour, day, _month, weekday] ->
        cond do
          ...
          day == "*" and weekday != "*" ->
            day_name = weekday_name(weekday)
            "#{day_name} at #{format_cron_time_short(hour, minute)}"
  ...
  defp format_cron_time(hour, minute) do
  ...
  defp format_cron_time_short(hour, minute) do
    case Integer.parse(hour) do
      {h, ""} ->
        m = String.pad_leading(minute, 2, "0")
        "#{h}:#{m}"
